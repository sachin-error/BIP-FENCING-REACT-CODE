...
export default function Attendance() {
  ...
  const [activeTab, setActiveTab]         = useState('all');   // all | today | week
  ...
  // auto-calc work hours when check-in/out changes
  useEffect(() => {
  ...
  const filtered = records.filter(r => {
    ...
    const matchQ = !q || r.employeeName.toLowerCase().includes(q)
            ...
            <div className="modal-content shadow-lg border-0 rounded-3">
              <div className="modal-header border-0">
                <h6 className="modal-title text-danger fw-bold">
                  <i className="bi bi-exclamation-triangle-fill me-2"></i>Delete Attendance Record
                </h6>
              </div>
              <div className="modal-body pt-0">
                <p className="mb-0 text-muted">
                  Remove attendance for <strong className="text-dark">{deleteConfirm.employeeName}</strong> on <strong className="text-dark">{deleteConfirm.date}</strong>?
                  This action is permanent.
                </p>
              </div>
              <div className="modal-footer border-0">
                <button className="btn btn-sm btn-outline-secondary rounded-pill px-4" onClick={() => setDeleteConfirm(null)}>Cancel</button>
                <button className="btn btn-sm btn-danger rounded-pill px-4" onClick={() => handleDelete(deleteConfirm.id)}>
        ...
        <div>
          <div className="d-flex align-items-center gap-2 mb-1">
            <div style={{ width: 36, height: 36, borderRadius: 10, background: 'linear-gradient(135deg,#1e40af,#3b82f6)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <i className="bi bi-calendar-check text-white" style={{ fontSize: 16 }}></i>
            </div>
      ...
      {showForm && (
        <div className="card border-0 mb-4" style={{ borderRadius: 16, boxShadow: '0 1px 3px rgba(0,0,0,0.06), 0 8px 24px rgba(0,0,0,0.06)' }}>
          <div className="card-header bg-white border-0 pb-0 pt-4 px-4" style={{ borderRadius: '16px 16px 0 0' }}>
            <div className="d-flex align-items-center gap-2">
              <div style={{ width: 30, height: 30, borderRadius: 8, background: editingId ? '#fef3c7' : '#eff6ff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <i className={`bi ${editingId ? 'bi-pencil-fill' : 'bi-person-check-fill'}`} style={{ fontSize: 13, color: editingId ? '#d97706' : '#1e40af' }}></i>
              </div>
          ...
          <div className="card-body px-4 pb-4">
            {submitted && (
              <div className="alert border-0 py-2 mb-3 d-flex align-items-center gap-2" style={{ background: '#f0fdf4', color: '#15803d', borderRadius: 10, fontSize: 13 }}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="row g-3 mb-4">
                <div className="col-md-4">
                  ...
                  <input className="form-control" style={{ borderRadius: 10, fontSize: 13 }} name="employeeName" value={form.employeeName} onChange={handleChange} placeholder="Full name" required />
                </div>
                <div className="col-md-3">
                  <label className="form-label" style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>Employee ID</label>
                  <input className="form-control" style={{ borderRadius: 10, fontSize: 13 }} name="employeeId" value={form.employeeId} onChange={handleChange} placeholder="EMP-001" />
                </div>
                <div className="col-md-3">
                  <label className="form-label" style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>Department</label>
                  <select className="form-select" style={{ borderRadius: 10, fontSize: 13 }} name="department" value={form.department} onChange={handleChange}>
                    <option value="">Select...</option>
                    {DEPARTMENTS.map(d => <option key={d}>{d}</option>)}
                  </select>
                </div>
                <div className="col-md-2">
                  <label className="form-label" style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>Shift</label>
                  <select className="form-select" style={{ borderRadius: 10, fontSize: 13 }} name="shift" value={form.shift} onChange={handleChange}>
                    <option value="">Select...</option>
              ...
              <div className="row g-3 mb-4">
                <div className="col-md-2">
                  <label className="form-label" style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>Date <span className="text-danger">*</span></label>
                  <input type="date" className="form-control" style={{ borderRadius: 10, fontSize: 13 }} name="date" value={form.date} onChange={handleChange} required />
                </div>
                <div className="col-md-3">
                  <label className="form-label" style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>Status <span className="text-danger">*</span></label>
                  <select className="form-select" style={{ borderRadius: 10, fontSize: 13 }} name="status" value={form.status} onChange={handleChange}>
                    {STATUSES.map(s => <option key={s}>{s}</option>)}
                  </select>
                </div>
                <div className="col-md-2">
                  <label className="form-label" style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>Check In</label>
                  <input type="time" className="form-control" style={{ borderRadius: 10, fontSize: 13 }} name="checkIn" value={form.checkIn} onChange={handleChange} />
                </div>
                <div className="col-md-2">
                  <label className="form-label" style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>Check Out</label>
                  <input type="time" className="form-control" style={{ borderRadius: 10, fontSize: 13 }} name="checkOut" value={form.checkOut} onChange={handleChange} />
                </div>
                <div className="col-md-2">
                  <label className="form-label" style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>Work Hours</label>
                  <div className="form-control d-flex align-items-center" style={{ borderRadius: 10, fontSize: 13, background: form.workHours ? '#f0fdf4' : '#f8fafc', color: '#15803d', fontWeight: 700 }}>
                    {form.workHours ? `${form.workHours} hrs` : '—'}
                  </div>
                </div>
                <div className="col-md-1">
                  <label className="form-label" style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>OT Hrs</label>
                  <input type="number" className="form-control" style={{ borderRadius: 10, fontSize: 13 }} name="overtime" value={form.overtime} onChange={handleChange} min="0" step="0.5" />
                </div>
              </div>
              ...
              <div className="row g-3 mb-4">
                {(form.status === 'On Leave') && (
                  <div className="col-md-3">
                    <label className="form-label" style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>Leave Type</label>
                    <select className="form-select" style={{ borderRadius: 10, fontSize: 13 }} name="leaveType" value={form.leaveType} onChange={handleChange}>
                      <option value="">Select...</option>
                ...
                <div className="col-md-3">
                  <label className="form-label" style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>Site / Location</label>
                  <input className="form-control" style={{ borderRadius: 10, fontSize: 13 }} name="siteLocation" value={form.siteLocation} onChange={handleChange} placeholder="Al Quoz, Site B..." />
                </div>
                <div className="col-md-3">
                  <label className="form-label" style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>Approved By</label>
                  <input className="form-control" style={{ borderRadius: 10, fontSize: 13 }} name="approvedBy" value={form.approvedBy} onChange={handleChange} placeholder="Manager name" />
                </div>
                <div className="col-md-4">
                  <label className="form-label" style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>Task / Work Description</label>
                  <input className="form-control" style={{ borderRadius: 10, fontSize: 13 }} name="taskDescription" value={form.taskDescription} onChange={handleChange} placeholder="Gate installation, fencing work..." />
                </div>
                <div className="col-12">
                  <label className="form-label" style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>Notes</label>
                  <textarea className="form-control" style={{ borderRadius: 10, fontSize: 13 }} name="notes" value={form.notes} onChange={handleChange} rows={2} placeholder="Additional remarks..." />
                </div>
              </div>
              ...
              <div className="d-flex gap-2">
                <button type="submit" className="btn rounded-pill px-4 fw-semibold"
                  ...
                  <i className="bi bi-check-lg me-2"></i>{editingId ? 'Update Record' : 'Save Attendance'}
                </button>
                <button type="button" className="btn btn-outline-secondary rounded-pill px-4" style={{ fontSize: 13 }} onClick={() => setForm(emptyForm)}>
                  <i className="bi bi-arrow-counterclockwise me-2"></i>Reset
                </button>
                {editingId && (
                  <button type="button" className="btn btn-outline-danger rounded-pill px-4" style={{ fontSize: 13 }} onClick={closeForm}>
                    Cancel Edit
                  </button>
                )}
              </div>
        ...
        <div className="card-header bg-white border-0 px-4 pt-4 pb-0" style={{ borderRadius: '16px 16px 0 0' }}>
          <div className="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-3">
            <div>
              ...
              <p className="text-muted mb-0" style={{ fontSize: 12 }}>{filtered.length} record{filtered.length !== 1 ? 's' : ''} shown</p>
            </div>
            <div className="d-flex gap-2 flex-wrap">
              <input type="text" className="form-control form-control-sm" placeholder="Search name / ID / dept / site..."
              ...
              <input type="date" className="form-control form-control-sm" value={filterDate}
                onChange={e => setFilterDate(e.target.value)} style={{ width: 140, borderRadius: 8, fontSize: 12 }} />
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-5">
              <div style={{ width: 64, height: 64, borderRadius: 16, background: '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
                <i className="bi bi-calendar-x text-muted" style={{ fontSize: 28 }}></i>
              </div>
                ...
                <thead>
                  <tr style={{ background: '#f8fafc', borderTop: '1px solid #f1f5f9' }}>
                    {['Employee','Dept','Date','Shift','Check In','Check Out','Work Hrs','OT','Status','Leave Type','Site / Location','Task','Approved By','Notes','Actions'].map(h => (
                      <th key={h} className="px-3 py-3 fw-semibold" style={{ color: '#64748b', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.05em', whiteSpace: 'nowrap', borderBottom: '1px solid #f1f5f9' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {filtered.map((rec, idx) => {
                    ...
                    return (
                      <tr key={rec.id} style={{ borderBottom: '1px solid #f8fafc', background: idx % 2 === 0 ? '#fff' : '#fafbfc' }}>
                        ...
                        <td className="px-3" style={{ color: '#475569', fontFamily: 'monospace' }}>{rec.checkIn || '—'}</td>
                        <td className="px-3" style={{ color: '#475569', fontFamily: 'monospace' }}>{rec.checkOut || '—'}</td>
                        ...
                        <td className="px-3" style={{ maxWidth: 130, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: '#475569', fontSize: 11 }} title={rec.taskDescription}>{rec.taskDescription || '—'}</td>
                        <td className="px-3" style={{ color: '#475569', fontSize: 11 }}>{rec.approvedBy || '—'}</td>
                        <td className="px-3" style={{ maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: '#94a3b8', fontSize: 11 }} title={rec.notes}>{rec.notes || '—'}</td>
                        <td className="px-3">
                          <div className="d-flex gap-1">
        ...
        <div className="card-footer bg-white border-0 px-4 py-3" style={{ borderRadius: '0 0 16px 16px' }}>
          <div className="d-flex justify-content-between align-items-center flex-wrap gap-2">
            <span style={{ fontSize: 12, color: '#94a3b8' }}>
              Showing <strong style={{ color: '#475569' }}>{filtered.length}</strong> of <strong style={{ color: '#475569' }}>{records.length}</strong> total records
            </span>
              ...
              {STATUSES.map(s => {
                ...
                return (
                  <span key={s} className="d-flex align-items-center gap-1" style={{ fontSize: 11, color: '#64748b' }}>
                    <i className={`bi ${m.icon} text-${m.color === 'orange' ? 'warning' : m.color}`}></i>
                    {cnt} {s}
                  </span>
                );
              })}
