...
const recordToForm = (rec) => ({
  ...
  date:            rec.ot_date           || rec.date            || '',
  department:      rec.department        || '',
  otType:          rec.ot_type           || rec.otType          || '',
  shift:           rec.shift             || 'Day',
  regularHours:    rec.regular_hours     || rec.regularHours    || 8,
  otHours:         rec.ot_hours          || rec.otHours         || '',
  otRate:          rec.ot_rate           || rec.otRate          || '1.5',
  basicHourlyRate: rec.basic_hourly_rate || rec.basicHourlyRate || '',
  reason:          rec.reason            || '',
...
export default function OT() {
  ...
  const loadOTRecords = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE_URL}/api/ot`, {
        headers: {
          'Authorization': `Bearer ${authToken}`,
          'Content-Type': 'application/json'
        }
      });
      if (!response.ok) throw new Error('Failed to load OT records');
      const data = await response.json();
      setRecords(data.data || []);
      setError('');
    } catch (err) {
      setError('Failed to load OT records: ' + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const otData = {
        ...
        ot_type:          form.otType,
        shift:            form.shift,
        regular_hours:    Number(form.regularHours),
        ot_hours:         Number(form.otHours),
        ot_rate:          Number(form.otRate),
        basic_hourly_rate: Number(form.basicHourlyRate),
        reason:           form.reason,
      ...
      if (editingId) {
        const response = await fetch(`${API_BASE_URL}/api/ot/${editingId}`, {
          method: 'PUT',
          headers: { 'Authorization': `Bearer ${authToken}`, 'Content-Type': 'application/json' },
          body: JSON.stringify(otData),
        });
        if (!response.ok) throw new Error('Failed to update OT record');
      } else {
        const response = await fetch(`${API_BASE_URL}/api/ot`, {
          method: 'POST',
          headers: { 'Authorization': `Bearer ${authToken}`, 'Content-Type': 'application/json' },
        ...
        if (!response.ok) throw new Error('Failed to create OT record');
      }
      ...
      setError('Failed to save OT record: ' + err.message);
    }
  };
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/ot/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${authToken}`, 'Content-Type': 'application/json' },
      });
      if (!response.ok) throw new Error('Failed to delete OT record');
      await loadOTRecords();
      setDeleteConfirm(null);
    } catch (err) {
      setError('Failed to delete OT record: ' + err.message);
    }
  };

  const filtered = records.filter(r => {
    const q = search.toLowerCase();
    const name  = (r.name || r.employeeName || '').toLowerCase();
    const empId = (r.employee_id || r.employeeId || '').toLowerCase();
  ...
  const totalOTHours  = records.reduce((s, r) => s + (Number(r.ot_hours  || r.otHours)  || 0), 0);
  const totalOTPay    = records.reduce((s, r) => s + (Number(r.ot_pay    || r.otPay)    || 0), 0);
    ...
    <div className="container-fluid p-4" style={{ backgroundColor: '#f8f9fa', minHeight: '100vh' }}>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content shadow-lg">
              ...
              <div className="modal-body">
                <p className="mb-0">
                  Delete OT record for <strong>{deleteConfirm.name || deleteConfirm.employeeName}</strong> on{' '}
                  <strong>{deleteConfirm.ot_date || deleteConfirm.date}</strong>? This cannot be undone.
                </p>
              </div>
      ...
      {loading && (
        <div className="text-center py-4">
          ...
          <p className="text-muted mt-2">Loading OT records...</p>
        </div>
      )}
              ...
              <table className="table table-hover align-middle mb-0" style={{ fontSize: 13 }}>
                ...
                <tbody>
                  {filtered.map(rec => {
                    ...
                    const recDate   = rec.ot_date        || rec.date            || '—';
                    const otType    = rec.ot_type        || rec.otType          || '—';
                    const regHrs    = rec.regular_hours  || rec.regularHours    || 0;
                    const otHrs     = rec.ot_hours       || rec.otHours         || 0;
                    const otRate    = rec.ot_rate        || rec.otRate          || 0;
                    const hrRate    = rec.basic_hourly_rate || rec.basicHourlyRate || 0;
                    const otPayVal  = rec.ot_pay         || rec.otPay           || 0;
                    const approvedBy = rec.approved_by   || rec.approvedBy      || '—';
                    return (
                      <tr key={rec.id}>
                        <td>
                          <div className="fw-semibold">{empName}</div>
                          <div className="text-muted" style={{ fontSize: 11 }}>{empId}</div>
                        </td>
                        <td>{rec.department || '—'}</td>
                        <td style={{ whiteSpace: 'nowrap' }}>{recDate}</td>
                        <td>{otType}</td>
                        <td>
                        ...
                        <td>{regHrs} hrs</td>
                        <td className="fw-semibold">{otHrs} hrs</td>
                        <td>{otRate}x</td>
                        <td>INR {Number(hrRate).toFixed(2)}</td>
                        <td className="fw-bold" style={{ color: '#bc4c00', fontFamily: 'monospace' }}>
                          INR {Number(otPayVal).toFixed(2)}
                        </td>
                ...
                <tfoot className="table-light">
                  <tr>
                    ...
                    <td className="fw-bold">
                      {filtered.reduce((s, r) => s + (Number(r.ot_hours || r.otHours) || 0), 0).toFixed(1)} hrs
                    </td>
                    <td colSpan={2}></td>
                    <td className="fw-bold" style={{ color: '#bc4c00', fontFamily: 'monospace' }}>
                      INR {filtered.reduce((s, r) => s + (Number(r.ot_pay || r.otPay) || 0), 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </td>
                    <td colSpan={4}></td>
                  </tr>
                </tfoot>
