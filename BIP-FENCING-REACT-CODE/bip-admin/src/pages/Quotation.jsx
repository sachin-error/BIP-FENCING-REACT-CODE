...
const normalizeQuotation = (rec) => ({
  id: rec.id,
  quoteNo: rec.quote_no ?? rec.quoteNo ?? '',
  quoteDate: rec.quote_date ?? rec.quoteDate ?? '',
  validUntil: rec.valid_until ?? rec.validUntil ?? '',
  clientName: rec.client_name ?? rec.clientName ?? '',
  clientPhone: rec.client_phone ?? rec.clientPhone ?? '',
  clientEmail: rec.client_email ?? rec.clientEmail ?? '',
  taxPercent: rec.tax_percent ?? rec.taxPercent ?? 0,
  ...
  discountAmt: rec.discount_amt ?? rec.discountAmt ?? 0,
  taxableAmt: rec.taxable_amt ?? rec.taxableAmt ?? 0,
  cgstAmt: rec.cgst_amt ?? rec.cgstAmt ?? 0,
  sgstAmt: rec.sgst_amt ?? rec.sgstAmt ?? 0,
  total: rec.total ?? 0,
});
...
function calcTotals(form) {
  ...
  if (hasItems) {
    ...
    const halfTax      = (taxableAmt * Number(form.taxPercent || 0)) / 200; // split equally CGST/SGST
  ...
  const discountAmt = Number(form.discount_amt ?? form.discountAmt ?? 0);
  const taxableAmt = Number(form.taxable_amt ?? form.taxableAmt ?? 0);
  const halfTax = Number(form.cgst_amt ?? form.cgstAmt ?? 0);
  const taxAmt = Number(form.cgst_amt ?? form.cgstAmt ?? 0) + Number(form.sgst_amt ?? form.sgstAmt ?? 0);
...
export default function Quotation() {
  ...
  useEffect(() => {
    const loadQuotations = async () => {
      try {
        ...
        setRecords(data.data ? data.data.map(normalizeQuotation) : []);
        setError('');
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const fetchQuotations = async () => {
      ...
      try {
        const response = await fetch(`${API_BASE_URL}/api/quotations?page=1&per_page=100`, {
        ...
        setRecords(result.data.map(normalizeQuotation));
        setView('table');
      } catch (fetchError) {
        setError(fetchError.message || 'Unable to load quotations');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = editId !== null
        ? `${API_BASE_URL}/api/quotations/${editId}`
      ...
      const result = await response.json();
      if (!response.ok || !result.success) {
        const message = result.message || (result.errors ? Object.values(result.errors).join(', ') : 'Unable to save quotation');
        throw new Error(message);
      }

      const saved = normalizeQuotation(result.data);
      if (editId !== null) {
        setRecords(records.map(r => r.id === editId ? saved : r));
        setEditId(null);
      } else {
  ...
  return (
    <>
      ...
      {error && (
        <div style={{ marginBottom: 18, color: '#842029', background: '#f8d7da', borderRadius: 10, padding: '12px 16px' }}>
      ...
      {loading && (
        <div style={{ marginBottom: 18, color: '#0f5132', background: '#d1e7dd', borderRadius: 10, padding: '12px 16px' }}>
          <i className="bi bi-arrow-repeat me-2"></i>
          Loading quotations from the backend…
        </div>
      )}
