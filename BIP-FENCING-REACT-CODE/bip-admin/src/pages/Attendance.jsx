...
// helper: map API snake_case record → form camelCase
const recordToForm = (rec) => ({
  ...
  leaveType:       rec.leave_type       || rec.leaveType       || '',
  workHours:       rec.work_hours       || rec.workHours       || '',
  overtime:        rec.overtime_hours   || rec.overtime        || '0',
  taskDescription: rec.task_description || rec.taskDescription || '',
  siteLocation:    rec.site_location    || rec.siteLocation    || '',
  approvedBy:      rec.approved_by      || rec.approvedBy      || '',
  notes:           rec.notes            || '',
});
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', borderRadius: 7, border: '1px solid #e5e7eb', overflow: 'hidden', flexShrink: 0 }}>
        {['AM', 'PM'].map(period => (
          <button key={period} type="button" onClick={() => update('ampm', period)}
            style={{
...
export default function Attendance() {
  ...
  const loadAttendance = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE_URL}/api/attendance?date=&per_page=500`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const attendanceData = {
        ...
        work_hours:       Number(form.workHours),
        overtime_hours:   Number(form.overtime),
      ...
      if (editingId) {
        const response = await fetch(`${API_BASE_URL}/api/attendance/${editingId}`, {
          method: 'PUT',
          headers: { 'Authorization': `Bearer ${authToken}`, 'Content-Type': 'application/json' },
      ...
      setError('Failed to save attendance record: ' + err.message);
    }
  };
  ...
  const filtered = records.filter(r => {
    ...
    const recDate = r.attendance_date || r.date || '';
    const matchQ  = !q
      || (r.name || r.employeeName || '').toLowerCase().includes(q)
      || (r.employee_id || r.employeeId || '').toLowerCase().includes(q)
      || (r.department || '').toLowerCase().includes(q)
      || (r.site_location || r.siteLocation || '').toLowerCase().includes(q);
    const matchS   = !filterStatus || r.status === filterStatus;
    const matchD   = !filterDept   || r.department === filterDept;
    const matchDt  = !filterDate   || recDate === filterDate;
    const matchTab = activeTab === 'all'   ? true
                   : activeTab === 'today' ? recDate === todayStr
                   : recDate >= weekAgoStr;
    return matchQ && matchS && matchD && matchDt && matchTab;
  });

  const total       = records.length;
  const todayCount  = records.filter(r => (r.attendance_date || r.date) === todayStr).length;
            ...
            <div className="modal-content shadow-lg border-0 rounded-3">
              ...
              <div className="modal-body pt-0">
                <p className="mb-0 text-muted">
                  ...
                  <strong className="text-dark">{deleteConfirm.name || deleteConfirm.employeeName}</strong> on{' '}
                  <strong className="text-dark">{deleteConfirm.attendance_date || deleteConfirm.date}</strong>? This action is permanent.
                </p>
              </div>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="row g-3 mb-4">
                <div className="col-md-4">
                  <label className="form-label" style={lbl}>Employee Name <span className="text-danger">*</span></label>
                  <input className="form-control" style={inp} name="employeeName" value={form.employeeName} onChange={handleChange} placeholder="Full name" required />
                </div>
                <div className="col-md-3">
                  <label className="form-label" style={lbl}>Employee ID</label>
                  <input className="form-control" style={inp} name="employeeId" value={form.employeeId} onChange={handleChange} placeholder="EMP-001" />
                </div>
                <div className="col-md-3">
                  <label className="form-label" style={lbl}>Department</label>
                  <select className="form-select" style={inp} name="department" value={form.department} onChange={handleChange}>
                ...
                <div className="col-md-2">
                  ...
                  <select className="form-select" style={inp} name="shift" value={form.shift} onChange={handleChange}>
              ...
              <div className="row g-3 mb-4">
                <div className="col-md-2">
                  ...
                  <input type="date" className="form-control" style={inp} name="date" value={form.date} onChange={handleChange} required />
                </div>
                <div className="col-md-3">
                  <label className="form-label" style={lbl}>Status <span className="text-danger">*</span></label>
                  <select className="form-select" style={inp} name="status" value={form.status} onChange={handleChange}>
                ...
                <div className="col-md-2">
                  ...
                  <div className="form-control d-flex align-items-center" style={{ ...inp, background: form.workHours ? '#f0fdf4' : '#f8fafc', color: form.workHours ? '#15803d' : '#9ca3af', fontWeight: 700, cursor: 'default' }}>
                    {form.workHours ? `${form.workHours} hrs` : '—'}
                  </div>
                </div>
                <div className="col-md-1">
                  <label className="form-label" style={lbl}>OT Hrs</label>
                  <input type="number" className="form-control" style={inp} name="overtime" value={form.overtime} onChange={handleChange} min="0" step="0.5" />
                </div>
              </div>
              ...
              <div className="row g-3 mb-4">
                {form.status === 'On Leave' && (
                  <div className="col-md-3">
                    ...
                    <select className="form-select" style={inp} name="leaveType" value={form.leaveType} onChange={handleChange}>
                ...
                <div className="col-md-3">
                  ...
                  <input className="form-control" style={inp} name="siteLocation" value={form.siteLocation} onChange={handleChange} placeholder="Al Quoz, Site B..." />
                </div>
                <div className="col-md-3">
                  <label className="form-label" style={lbl}>Approved By</label>
                  <input className="form-control" style={inp} name="approvedBy" value={form.approvedBy} onChange={handleChange} placeholder="Manager name" />
                </div>
                <div className="col-md-4">
                  <label className="form-label" style={lbl}>Task / Work Description</label>
                  <input className="form-control" style={inp} name="taskDescription" value={form.taskDescription} onChange={handleChange} placeholder="Gate installation, fencing work..." />
                </div>
        ...
        <div className="card-header bg-white border-0 px-4 pt-4 pb-0" style={{ borderRadius: '16px 16px 0 0' }}>
          <div className="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-3">
            ...
            <div className="d-flex gap-2 flex-wrap">
              ...
              <select className="form-select form-select-sm" value={filterStatus} onChange={e => setFilterStatus(e.target.value)} style={{ width: 140, borderRadius: 8, fontSize: 12 }}>
                <option value="">All Status</option>
                {STATUSES.map(s => <option key={s}>{s}</option>)}
              </select>
              <select className="form-select form-select-sm" value={filterDept} onChange={e => setFilterDept(e.target.value)} style={{ width: 155, borderRadius: 8, fontSize: 12 }}>
              ...
              <table className="table align-middle mb-0" style={{ fontSize: 12.5 }}>
                ...
                <tbody>
                  {filtered.map((rec, idx) => {
                    ...
                    const empName       = rec.name             || rec.employeeName    || '—';
                    const empId         = rec.employee_id      || rec.employeeId      || '—';
                    const recDate       = rec.attendance_date  || rec.date            || '—';
                    const checkIn       = rec.check_in         || rec.checkIn         || '';
                    const checkOut      = rec.check_out        || rec.checkOut        || '';
                    const workHrs       = rec.work_hours       || rec.workHours       || 0;
                    const otHrs         = rec.overtime_hours   || rec.overtime        || 0;
                    const leaveType     = rec.leave_type       || rec.leaveType       || '—';
                    const siteLocation  = rec.site_location    || rec.siteLocation    || '—';
                    const taskDesc      = rec.task_description || rec.taskDescription || '—';
                    const approvedBy    = rec.approved_by      || rec.approvedBy      || '—';
                    return (
                      <tr key={rec.id} style={{ borderBottom: '1px solid #f8fafc', background: idx % 2 === 0 ? '#fff' : '#fafbfc' }}>
                        <td className="px-3 py-3">
                          <div className="d-flex align-items-center gap-2">
                            <div style={{ width: 32, height: 32, borderRadius: 8, background: `hsl(${(empName.charCodeAt(0) * 7) % 360},70%,92%)`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                              <span style={{ fontSize: 11, fontWeight: 700, color: `hsl(${(empName.charCodeAt(0) * 7) % 360},60%,35%)` }}>
                                {empName.split(' ').slice(0,2).map(w=>w[0]).join('').toUpperCase()}
                              </span>
                            </div>
                            <div>
                              <div className="fw-semibold" style={{ color: '#0f172a', fontSize: 13 }}>{empName}</div>
                              <div style={{ color: '#94a3b8', fontSize: 11 }}>{empId}</div>
                            </div>
                          </div>
                        </td>
                        <td className="px-3" style={{ color: '#475569' }}>{rec.department || '—'}</td>
                        <td className="px-3" style={{ whiteSpace: 'nowrap', color: '#475569', fontWeight: 600 }}>{recDate}</td>
                        <td className="px-3" style={{ color: '#475569', whiteSpace: 'nowrap', fontSize: 11 }}>{rec.shift || '—'}</td>
                        <td className="px-3" style={{ color: '#475569', fontFamily: 'monospace', whiteSpace: 'nowrap' }}>{checkIn ? formatTime12(checkIn) : '—'}</td>
                        <td className="px-3" style={{ color: '#475569', fontFamily: 'monospace', whiteSpace: 'nowrap' }}>{checkOut ? formatTime12(checkOut) : '—'}</td>
                        <td className="px-3 fw-bold" style={{ color: '#15803d', fontFamily: 'monospace' }}>{workHrs ? `${workHrs}h` : '—'}</td>
                        <td className="px-3" style={{ color: Number(otHrs) > 0 ? '#d97706' : '#94a3b8', fontWeight: Number(otHrs) > 0 ? 700 : 400 }}>
                          {Number(otHrs) > 0 ? `+${otHrs}h` : '—'}
                        </td>
                        <td className="px-3">
                          <span className={`badge bg-${meta.color === 'orange' ? 'warning' : meta.color} bg-opacity-10 text-${meta.color === 'orange' ? 'warning' : meta.color} d-inline-flex align-items-center gap-1`}
                            ...
                            <i className={`bi ${meta.icon}`} style={{ fontSize: 10 }} />{rec.status}
                          </span>
                        </td>
                        <td className="px-3" style={{ color: '#475569', fontSize: 11 }}>{leaveType}</td>
                        <td className="px-3" style={{ color: '#475569', fontSize: 11, whiteSpace: 'nowrap' }}>{siteLocation}</td>
                        <td className="px-3" style={{ maxWidth: 130, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: '#475569', fontSize: 11 }} title={taskDesc}>{taskDesc}</td>
                        <td className="px-3" style={{ color: '#475569', fontSize: 11 }}>{approvedBy}</td>
                ...
                <tfoot>
                  <tr style={{ background: '#f8fafc', borderTop: '2px solid #e2e8f0' }}>
                    ...
                    <td className="px-3 fw-bold" style={{ color: '#15803d', fontFamily: 'monospace', fontSize: 12 }}>
                      {filtered.reduce((s, r) => s + (Number(r.work_hours || r.workHours) || 0), 0).toFixed(1)}h
                    </td>
                    <td className="px-3 fw-bold" style={{ color: '#d97706', fontFamily: 'monospace', fontSize: 12 }}>
                      {filtered.reduce((s, r) => s + (Number(r.overtime_hours || r.overtime) || 0), 0).toFixed(1)}h OT
                    </td>
                    <td colSpan={7} />
                  </tr>
                </tfoot>
