...
function normalize(r) {
  return {
    ...
    otAmount:     r.ot_amount    ?? r.otAmount    ?? 0,
    bonus:        r.bonus        ?? 0,
    deductions:   r.deductions   ?? 0,
    taxDeduction: r.tax_deduction ?? r.taxDeduction ?? 0,
    paymentMode:  r.payment_mode ?? r.paymentMode ?? 'Bank Transfer',
    notes:        r.notes        ?? '',
  };
}
  ...
  useEffect(() => {
    const loadSalaries = async () => {
      try {
        ...
        setRecords((data.data || []).map(normalize));
        setError('');
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const salaryData = {
        ...
        employee_id: form.employeeId.trim() || 'EMP-' + form.employeeName.trim().substring(0,3).toUpperCase() + '-' + Date.now().toString().slice(-4),
        ...
        ot_amount: Number(form.otAmount),
        bonus: Number(form.bonus),
        deductions: Number(form.deductions),
        tax_deduction: Number(form.taxDeduction),
      ...
      if (editId !== null) {
        ...
        const response = await fetch(`${API_BASE_URL}/api/salary/${editId}`, {
          method: 'PUT',
          headers: {
            'Authorization': `Bearer ${authToken}`,
            'Content-Type': 'application/json'
        ...
        if (!response.ok) {
          let message = 'Failed to update salary record';
          try {
        ...
        if (!response.ok) {
          let message = 'Failed to create salary record';
          try {
            const err = await response.json();
            message = err?.message || message;
          } catch (_) {}
      ...
      const loadResponse = await fetch(`${API_BASE_URL}/api/salary`, {
        headers: {
          'Authorization': `Bearer ${authToken}`,
          'Content-Type': 'application/json'
        }
      ...
      setError('Failed to save salary record: ' + err.message);
                      ...
                      <tr key={rec.id}>
                        ...
                        <td>
                          <div className="sal-actions" style={{ justifyContent:'flex-end' }}>
                            <button type="button" className="sal-btn-view" onClick={() => setViewRec(rec)}>
                              <i className="bi bi-eye" style={{ fontSize:12 }}></i> View
                            </button>
                            <button type="button" className="sal-btn-edit" onClick={() => handleEdit(rec)}>
                              <i className="bi bi-pencil" style={{ fontSize:12 }}></i> Edit
                            </button>
                            <button type="button" className="sal-btn-del" onClick={() => setDeleteId(rec.id)}>
