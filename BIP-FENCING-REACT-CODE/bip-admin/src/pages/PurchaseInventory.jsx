...
export default function PurchaseInventory() {
  ...
  // Load purchase inventory from API
  useEffect(() => {
    const loadInventory = async () => {
      try {
        setLoading(true);
        const response = await fetch(`${API_BASE_URL}/api/purchase-inventory`, {
          headers: {
        ...
        if (!response.ok) {
          throw new Error('Failed to load purchase inventory');
        }
        ...
        setError('Failed to load purchase inventory: ' + err.message);
        console.error('Error loading purchase inventory:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const nextPoNo = () => `PO-${String(Date.now()).slice(-6)}`;

  const [form, setForm] = useState({ ...EMPTY_FORM(), poNo: nextPoNo() });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        const response = await fetch(`${API_BASE_URL}/api/purchase-inventory/${editingId}`, {
          method: 'PUT',
          headers: {
            'Authorization': `Bearer ${authToken}`,
            'Content-Type': 'application/json'
      ...
      // Reload inventory
      const loadResponse = await fetch(`${API_BASE_URL}/api/purchase-inventory`, {
        headers: {
          'Authorization': `Bearer ${authToken}`,
          'Content-Type': 'application/json'
        }
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/purchase-inventory/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${authToken}`,
          'Content-Type': 'application/json'
