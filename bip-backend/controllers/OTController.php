...
class OTController {
    ...
    public function getAll() {
        try {
            ...
            $countSql    = "SELECT COUNT(*) as count FROM ot_records ot
                            JOIN employees e ON ot.employee_id = e.id WHERE 1=1";
            $countParams = [];
    ...
    public function getById($id) {
        try {
            ...
            if (!$record) Response::notFound('OT record not found');
    ...
    public function create() {
        try {
            ...
            // Resolve string employee_id (e.g. "EMP-001") to integer FK
            ...
            if (!$employee) {
                $employeeDbId = $employeeModel->create([
                    ...
                    'name'        => $input['employee_name'] ?? $input['employee_id'],
            ...
            $input['employee_id'] = $employeeDbId;
            unset($input['employee_name']); // not a DB column in ot_records
            unset($input['ot_pay']);        // GENERATED column
    ...
    public function update($id) {
        try {
            ...
            // Strip generated column and non-column fields
            unset($input['ot_pay'], $input['employee_name'], $input['name']);

            $this->otModel->updateOT($id, $input);
    ...
    public function updateStatus($id) {
        try {
            ...
            $input = json_decode(file_get_contents('php://input'), true);
            if (empty($input['status'])) Response::validationError(['status' => 'Status is required']);
